...
Choose whether chats start in Complete or Infinite mode. The composer can override this setting per chat.

```jsonc
{
  "default_session_mode": "infinite",
}
```
...
- `goalTracking` - Require the tracked OpenCode goal and objectives to be complete before stopping (default: `true`). `todoDetection` remains a deprecated compatibility alias.

Use the Mode toggle in the composer footer to switch between `Complete` and `Infinite` per session. Infinite creates a primary goal, maintains ordered objectives, and can add or revise objectives as it discovers more required work. The existing Stop button interrupts the loop at any time.
