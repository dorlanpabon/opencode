...
- Custom instructions are session-lifetime instructions: each Session keeps the value, including an empty value, captured by its initial instruction baseline; global edits apply only to Sessions initialized afterward.
